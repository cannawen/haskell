...
- Filter to only look at rectangles where all 4 corners are within polygon (performace optimization)
...
- [The point crosses an odd number of lines](https://en.wikipedia.org/wiki/Point_in_polygon#Ray_casting_algorithm)

## Ray casting

- Start with list of consecutive polygon points
- Create tuples of neighbouring points (wrapping around when you get to the end of the array), representing line segments
- Filter line segments for only vertical lines
- Obtain all intermediary points on the line, except for the very bottom point
    - i.e. line segment [(3,5), (7,5)] becomes [(3,5), (4,5), (5,5), (6,5)] and [(4,70), (6,70)] becomes [(4,70), (5,70)]
    - we exclude the bottom point because that's just how ray casting works
- Encode the list of all intermediary points to a map where the key represents a row (x value), and it stores a sorted array of columns in which the polygon border crosses it (y values)
...
Optimization: sort all points by x and then y values and then do a reduce over them to create our encoded data structure in O(nlogn) time
